...
class ListAssociationsCommandTest {
    ...
    void run_callsCorrectEndpoint_noFilters() {
        when(apiClient.get("/clusters/test-cluster/pod-identity-associations"))
            .thenReturn("{\"associations\":[]}");

        cmd.run();

        verify(apiClient).get("/clusters/test-cluster/pod-identity-associations");
    }

    @Test
    void run_appendsNamespaceFilter() {
        cmd.namespace = "kube-system";
        when(apiClient.get("/clusters/test-cluster/pod-identity-associations?namespace=kube-system"))
            .thenReturn("{\"associations\":[]}");

        cmd.run();

        verify(apiClient).get("/clusters/test-cluster/pod-identity-associations?namespace=kube-system");
    }
    ...
    void run_appendsBothFilters() {
        ...
        when(apiClient.get("/clusters/test-cluster/pod-identity-associations?namespace=default&serviceAccount=my-sa"))
            .thenReturn("{\"associations\":[]}");

        cmd.run();

        verify(apiClient).get("/clusters/test-cluster/pod-identity-associations?namespace=default&serviceAccount=my-sa");
    }
