...
5. Calls `eks-dx create cluster {tenantId} --issuer https://{publicIp} --jwks-file /tmp/jwks.json`
...
| | k3s | EKS-D-Xpress |
|---|---|---|
| Cluster registration | Manual (`eks-dx create cluster`) | Automatic (first-boot script) |
