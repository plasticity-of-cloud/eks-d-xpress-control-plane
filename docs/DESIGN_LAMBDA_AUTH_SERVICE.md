...
| **Webhook** | `GET /clusters/{name}/pod-identity-associations` | `eks-dx.codriverlabs.ai` | Valid signature + audience + subject is `system:serviceaccount:kube-system:eks-dx-pod-identity-webhook` |
...
The webhook mounts a projected SA token with audience `eks-dx.codriverlabs.ai`:
...
2. Checks audience = `eks-dx.codriverlabs.ai` (not `pods.eks.amazonaws.com`)
...
- **Scoped by audience** — a pod token (`pods.eks.amazonaws.com`) can't query the management API, and a webhook token (`eks-dx.codriverlabs.ai`) can't exchange credentials
