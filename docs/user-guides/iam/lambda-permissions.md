...
|-----------|---------------|--------|
| `dynamodb:GetItem` | `eks-d-xpress-clusters` table | Look up JWKS and issuer for JWT validation |
| `dynamodb:GetItem` | `eks-d-xpress-associations` table | Look up namespace/SA → roleArn mapping |
...
| `sts:SetSourceIdentity` | `*` | Propagate pod identity as source identity for CloudTrail |
...
| Permission | Resource Scope | Reason |
|-----------|---------------|--------|
| `dynamodb:GetItem`, `PutItem`, `DeleteItem`, `UpdateItem`, `Scan`, `Query` | `eks-d-xpress-clusters` table | Cluster registration, describe, list, deregister, JWKS update |
| `dynamodb:GetItem`, `PutItem`, `DeleteItem`, `UpdateItem`, `Scan`, `Query` | `eks-d-xpress-associations` table | Association CRUD |
| `iam:GetRole` | `arn:aws:iam::*:role/*` | Check if role exists before creating association |
| `iam:ListRoleTags` | `arn:aws:iam::*:role/*` | Verify role is tagged `eks-dx-managed=true` before trust policy update |
| `iam:UpdateAssumeRolePolicy` | `arn:aws:iam::<account>:role/*` (condition: tag `eks-dx-managed=true`) | Add/remove trust policy statements for pod identity associations |
...
| Permission | Resource Scope | Reason |
|-----------|---------------|--------|
| Full CRUD | `eks-d-xpress-tenants` table | Write tenant state, read progress, delete on teardown |
| Full CRUD | `eks-d-xpress-clusters` table | Pre-register cluster (JWKS + issuer), delete on teardown |

### KMS

| Permission | Resource Scope | Reason |
|-----------|---------------|--------|
| `kms:Sign` | `eks-d-xpress/control-plane/ca-signing-key` | Sign per-tenant CA certificate with shared HSM-backed key |

### Secrets Manager

| Permission | Resource Scope | Reason |
|-----------|---------------|--------|
| `secretsmanager:CreateSecret` | `arn:aws:secretsmanager:*:*:secret:eks-dx/tenant/*` | Store CA key, CA cert, SA key, SSH key |
...
|-----------|---------------|-----------|--------|
| `ec2:ReleaseAddress` | EIP ARN wildcard | `project=eks-d-xpress` | Release tenant EIP on teardown |
| `ec2:DisassociateAddress` | Same | Same | Disassociate EIP before release |
| `ec2:DeleteKeyPair` | Key pair ARN wildcard | `project=eks-d-xpress` | Delete SSH key pair on teardown |
| `ec2:DeleteSnapshot` | Snapshot ARN wildcard | `Platform=eks-d-xpress` | Delete DLM-created etcd snapshots on teardown |
| `ec2:AllocateAddress` | `*` | — | Allocate EIP for tenant instance |
| `ec2:AssociateAddress` | `*` | — | Bind EIP to instance |

### IAM (scoped to `eks-dx-tenant-*`)

| Permission | Resource Scope | Reason |
|-----------|---------------|--------|
| `iam:CreateRole` | `arn:aws:iam::*:role/eks-dx-tenant-*` | Per-tenant instance role |
...
| `iam:CreateInstanceProfile` | `arn:aws:iam::*:instance-profile/eks-dx-tenant-*` | Per-tenant instance profile |
...
| `dlm:TagResource` | `*` | Tag policy with `eks-dx-tenant` for lookup |

### SQS (scoped to `eks-dx-tenant-*`)

| Permission | Resource Scope | Reason |
|-----------|---------------|--------|
| `sqs:CreateQueue` | `arn:aws:sqs:<region>:<account>:eks-dx-tenant-*` | Karpenter interruption queue per cluster |
| `sqs:DeleteQueue` | Same | Teardown cleanup |
| `sqs:GetQueueUrl` | Same | Resolve queue URL for deletion |

### EventBridge (scoped to `eks-dx-tenant-*`)

| Permission | Resource Scope | Reason |
|-----------|---------------|--------|
| `events:PutRule` | `arn:aws:events:<region>:<account>:rule/eks-dx-tenant-*` | Spot interruption / rebalance rules |
...
| Permission | Resource Scope | Reason |
|-----------|---------------|--------|
| `ssm:GetParameter` | `/eks-d-xpress/control-plane/*`, `/eks-d-xpress/infra/ami/*` | Read API endpoint, AMI IDs at runtime |
...
2. **Resource scoping**: Mutating actions are scoped to `eks-dx-tenant-*` resource names or conditioned on tags.
3. **Tag-based conditions**: EIP/key-pair/snapshot deletion requires `project=eks-d-xpress` or `Platform=eks-d-xpress` tag — prevents accidental deletion of unrelated resources.
