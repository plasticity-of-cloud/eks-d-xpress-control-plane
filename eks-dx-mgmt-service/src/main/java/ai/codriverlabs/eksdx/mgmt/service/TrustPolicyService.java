...
/**
 * Manages trust policy statements on target roles for EKS-DX Pod Identity.
...
public class TrustPolicyService {
    ...
    private static final String EKS_DX_MANAGED_TAG = "eks-dx-managed";
    ...
    public TrustPolicyResult addTrustStatement(String roleArn, String clusterName,
        ...
        if (!isRoleTagged(roleName)) {
            LOG.infof("Role %s not tagged %s=true — returning manual action required", roleName, EKS_DX_MANAGED_TAG);
    ...
    public void validateTrustPolicy(String roleArn, String decodedTrustPolicy) {
        if (!decodedTrustPolicy.contains(BROKER_ROLE_NAME))
            throw new IllegalArgumentException(
                ...
                ". Tag the role with '" + EKS_DX_MANAGED_TAG + "=true' and EKS-DX will manage the trust policy automatically.");
    }
    ...
    private boolean isRoleTagged(String roleName) {
        try {
            ...
            return tagsResp.tags().stream()
                .anyMatch(t -> EKS_DX_MANAGED_TAG.equals(t.key()) && "true".equals(t.value()));
        } catch (Exception e) {
            return false;
        }
    }
    ...
    static String buildSid(String clusterName, String namespace, String serviceAccount) {
        ...
        String raw = "AllowEksDX" + clusterName + namespace + serviceAccount;
